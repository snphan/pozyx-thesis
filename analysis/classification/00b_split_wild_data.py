...
all_cleaned_data: dict[str, dict[str, pd.DataFrame]] = defaultdict(
    dict
)  # {name_of_file: pd.DataFrame}


def split_exp_to_actions(experiment):
    for data_fp in position_data_dir.joinpath(experiment).glob("*.csv"):
        try:
            label_fp = Path().joinpath(
                "data", "03_Labels", experiment, data_fp.name.replace(".csv", ".txt")
            )
            ...
            for action_name in actions:
                for ind, time_range in enumerate(actions[action_name]):
                    ...
                    if not output_dir.joinpath(action_name).exists():
                        output_dir.joinpath(action_name).mkdir(
                            parents=True, exist_ok=True
                        )
                    output_fn = (
                        data_fp.name.replace(".csv", "")
                        + "_"
                        + action_name
                        + "_"
                        + str(ind)
                    ...
                    if not output_dir.joinpath(action_name, output_fn).exists():
                        action_segment.to_csv(
                            output_dir.joinpath(action_name, output_fn)
                        )
...
for _ in tqdm(
    ThreadPool(cpus).imap_unordered(split_exp_to_actions, EXP_TYPES),
    total=len(EXP_TYPES),
    desc="Experiment",
):
    pass

...
The convolutional operation from one filter will produce an array with shape ($O$,$O$,1). For $k$ number of kernels that the user defines, the output shape from the will be ($O$,$O$,$k$).

After the convolution operation is complete, a bias $b$ is added to the output from each kernel and the values are typically passed through the ReLU activation function to remove the negative values \cite{tayeTheoreticalUnderstandingConvolutional2023,purwonoUnderstandingConvolutionalNeural2023}. In other words, the output for the $k$-th kernel $h^k$ (that is passed to the next layer) is as follows

\begin{equation}
   h^k = \sigma(W^k * x + b^k) 
\end{equation}

Where $W^k$ are the weights of the $k$-th kernel, $b^k$ is the bias added to the output of the $k$-th kernel, and the $*$ symbol represents the convolution operation.

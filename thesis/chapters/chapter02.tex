...
% TODO: Look at some of the references in intro https://www.mdpi.com/1660-4601/18/1/163 and add to here.
...
\section{Camp et al's Tech Used to Recognize ADL in Community-Dwelling Older Adults}
\subsection{Overview}
Camp et al.'s search identified a total of "21 [distinct] technologies, including six wearable sensors, 13 environmental sensors, and two types of camera" \cite{camp_technology_2021}. The full list can be found in Table \ref{tab:camps-tech}. These technologies were able to detect a total of 14 different ADLs, but no single technology system was able to detect all 14 \cite{camp_technology_2021}. Table \ref{tab:camps-ADL} summarizes the ADLs detected as well as their level of granularity. There were 39 systems in total identified and most used the environment sensors. Only 3 out of the 39 systems used wearables. However, these wearable systems were able to recognize granular activities within "feeding" such as drinking and using cutlery that was not included in the other \cite{camp_technology_2021}. 
...
The systems identified recognize ADLs by using a series of interactions with the environment sensors and/or wearable sensors. For example, "feeding" can be inferred by activity in the motion sensors in the kitchen/dining room, fridge door sensors, and power sensors attached to a microwave \cite{camp_technology_2021}. A full list of these interactions and accompanying ADL can be found in Table. Though these systems can recognize ADLs and is sufficient for the aim of monitoring functional decline by monitoring a decrease in ADL actions, Camp et al. suggest that methods to "directly [assess] ADL performance" should be considered as it will allow for identification of "exactly where an individual is having difficulty without the reliance on [time-consuming] traditional tests" \cite{camp_technology_2021}. 
...
\clearpage
\section{Gadey et al's Tech for Monitoring ADL in Older Adults}
% TODO: Talk about the following for each mentioned in the overview
% - Type of Tech
% - Parameters or metrics used
% - ADL used for
% - Criteria for each 
